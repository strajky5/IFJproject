...
# Black-box tests
for src in `ls $BlackBox | grep -E "*.src"`; do
    # Test name
    ...
    if [ "x$RET" == "x0" ]; then
        echo "DIFF: "
        cat $BlackBox$IN | "$interpret" $BlackBox$src | diff - $BlackBox$OUT
    else
        printf "  Return value: "
        cat $BlackBox$IN | "$interpret" $BlackBox$src 2>/dev/null 1>/dev/null
        RETURNED=$?
        if [ "x$RETURNED" == "x$RET" ]; then
            echo "[OK] ($RETURNED vs. $RET)"
        else
